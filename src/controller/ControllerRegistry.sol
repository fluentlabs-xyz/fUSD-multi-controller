...
contract ControllerRegistry is AccessControl {
    ...
    event ControllerUpdated(
        address indexed controller, string oldName, string newName, uint256 oldLimit, uint256 newLimit
    );
     ...
     */
    function updateController(address controller, string memory name, uint256 limit) external onlyRole(ADMIN_ROLE) {
        require(controllers[controller].active, "Registry: controller not found");
        require(bytes(name).length > 0, "Registry: empty name");
        require(limit > 0, "Registry: zero limit");
    ...
    function recordMint(address controller, uint256 amount) external onlyRole(ADMIN_ROLE) {
        ...
        require(getGlobalDailyMinted() + amount <= globalDailyLimit, "Registry: global daily limit exceeded");
